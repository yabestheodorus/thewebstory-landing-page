    ...
    <main className="relative bg-secondary text-ink min-h-screen">
      <div className="pt-32 pb-20">
        <div className="max-w-7xl mx-auto px-6">
          {/* Static Top Nav */}
          <div className="mb-12">
            <Link href={`/${lang}`} className="inline-flex items-center gap-2 label-meta hover:text-blaze transition-colors">
              <ArrowLeft size={16} />
              Back to Home
            </Link>
          </div>
          ...
          <div className="max-w-4xl mb-24 md:mb-40">
            <span className="label-meta text-blaze font-black tracking-[0.3em] uppercase mb-6 block">
          ...
          <div className="grid lg:grid-cols-[1fr_400px] gap-20 items-start">
            <div className="space-y-32">
              ...
              {s.problem_block && (
                <section>
                  <div className="flex items-center gap-3 mb-8">
                    <AlertCircle className="text-blaze w-6 h-6" />
                  ...
                  <div className="grid gap-6">
                    {s.problem_block.points.map((point: string, i: number) => (
                      <div key={i} className="flex gap-6 p-8 bg-card/40 rounded-3xl border border-ink/5 hover:border-blaze/10 transition-colors">
              ...
              {/* Solution Block */}
              {s.solution_block && (
                <section className="bg-blaze/5 text-ink p-10 md:p-20 rounded-[3rem] border border-blaze/10 relative overflow-hidden group">
                  <div className="absolute top-0 right-0 w-64 h-64 bg-blaze/5 blur-[100px] rounded-full group-hover:bg-blaze/10 transition-all duration-700" />
                ...
                <section>
                  ...
                  <div className="grid gap-4">
                    {(s.what_you_get?.items || s.what_you_get_detailed || []).map((item: any, i: number) => (
                      <div key={i} className="group p-8 border border-ink/5 hover:bg-blaze/5 hover:border-blaze/10 transition-all duration-500 rounded-3xl">
                        <div className="flex items-start gap-6">
                          <CheckCircle2 className="text-blaze w-6 h-6 mt-1 shrink-0" />
                          <div>
                            <h3 className="text-xl md:text-2xl font-bold mb-2 group-hover:text-blaze transition-colors">{item.label}</h3>
                ...
                <section>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {(s.supported_couriers || s.supported_channels).map((c: string) => (
                      <span key={c} className="px-6 py-3 bg-card/40 border border-ink/5 rounded-full font-bold text-sm hover:border-blaze/20 transition-colors">{c}</span>
                    ))}
                  </div>
                </section>
              )}
              ...
              {s.best_for && (
                <section className="bg-sand/30 p-10 md:p-16 rounded-[3rem] border border-ink/5 relative overflow-hidden">
                  <div className="absolute -bottom-10 -right-10 w-32 h-32 bg-blaze/5 blur-3xl rounded-full" />
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    {s.best_for.points.map((point: string, i: number) => (
                      <div key={i} className="flex items-start gap-3">
                        <div className="w-2 h-2 rounded-full bg-blaze mt-2.5 shrink-0" />
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {s.example_use_cases.map((use: string, i: number) => (
                      <div key={i} className="p-6 bg-card/20 border border-ink/5 rounded-2xl flex items-center gap-4 hover:bg-card/40 transition-colors">
                        <Info className="text-blaze w-5 h-5 shrink-0" />
              ...
              {s.faq && (
                <section>
                  <div className="flex items-center gap-3 mb-12">
                    <HelpCircle className="text-blaze w-8 h-8" />
                  ...
                  <div className="space-y-4">
                    {s.faq.map((item: any, i: number) => (
                      <div key={i} className="bg-white/40 p-8 rounded-[2rem] border border-ink/5 hover:border-blaze/10 transition-colors">
            ...
            <aside className="lg:sticky lg:top-32 space-y-6">
              <div className="bg-white p-10 rounded-[3rem] border border-ink/5 shadow-2xl shadow-ink/5 relative overflow-hidden flex flex-col h-full">
                <div className="absolute top-0 right-0 w-24 h-24 bg-blaze/5 blur-3xl rounded-full" />
                
                <div className="relative z-10">
                  {s.mood && (
                    <span className="label-meta text-blaze font-bold text-[10px] uppercase tracking-[0.2em] mb-4 block opacity-60 italic">{s.mood}</span>
                  )}
                  <span className="label-meta text-ink/20 font-bold uppercase tracking-widest mb-2 block">{dict.works.labels.price_start}</span>
                  <div className="mb-6">
                    {s.original_price && (
                      <span className="text-ink/20 line-through text-sm font-medium decoration-blaze/50 block mb-1">{s.original_price}</span>
                    )}
                  ...
                  <div className="flex items-center gap-2 mb-10">
                    <span className="label-meta text-blaze font-black text-[12px] uppercase tracking-[0.2em]">{s.delivery} delivery</span>
                  </div>
                  ...
                  <div className="space-y-6 mb-10">
                    ...
                    {s.perfect_excuse && (
                      <div className="flex gap-3">
                        <div className="w-1.5 h-1.5 rounded-full bg-blaze mt-1.5 shrink-0" />
                ...
                <div className="mt-auto relative z-10">
                  <Link
                    href={`/${lang}/contact`}
                    className="flex items-center justify-center gap-3 bg-ink text-off w-full py-6 rounded-full font-bold text-lg hover:bg-blaze hover:text-white transition-all duration-300 group mb-6"
                  >
                  ...
                  {s.fun_fact && (
                    <div className="pt-6 border-t border-ink/5">
                      <div className="flex items-start gap-3">
                        <Info size={14} className="text-blaze mt-0.5 shrink-0" />
                ...
                <ul className="space-y-4">
                  {dict.works.disclaimer.map((d: string, i: number) => (
                    <li key={i} className="flex gap-3 items-start">
                      <CheckCircle2 size={16} className="text-blaze mt-1.5 shrink-0" />
